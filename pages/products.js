        ...
        <div className="bg-white rounded-2xl p-8 shadow text-center">
          ...
          >
            <div className="aspect-square bg-gray-100 rounded-lg mb-4 flex items-center justify-center overflow-hidden">
              <img
                src={`/images/${p.id}/thumbnail.jpg`}
                alt={p.englishName}
                className="w-full h-full object-cover rounded-lg"
                onError={(e) => {
                  const currentSrc = e.target.src;
                  // Prevent infinite loop by tracking what we've tried
                  if (currentSrc.includes('thumbnail.jpg')) {
                    e.target.src = `/images/${p.id}/thumbnail.png`;
                  } else if (currentSrc.includes('thumbnail.png')) {
                    e.target.src = `/images/${p.id}/image_1.jpg`;
                  } else if (currentSrc.includes('image_1.jpg')) {
                    e.target.src = `/images/${p.id}/image_1.png`;
                  } else if (currentSrc.includes('image_1.png')) {
                    e.target.src = `/images/${p.id}/main.jpg`;
                  } else if (currentSrc.includes('main.jpg')) {
                    e.target.src = `/images/${p.id}/main.png`;
                  } else if (currentSrc.includes('main.png')) {
                    e.target.src = '/placeholder-product.png';
                  } else if (!currentSrc.includes('placeholder-product.png')) {
                    e.target.src = '/placeholder-product.png';
                  }
                }}
                onLoad={(e) => {
